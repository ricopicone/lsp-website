"""Tests for the site-theme skin (modern default vs opt-in wix/classic)."""
...
def test_default_is_modern():
    request = SimpleNamespace(COOKIES={})
    assert site_theme(request) == {"site_theme": "modern"}


def test_cookie_selects_wix():
    request = SimpleNamespace(COOKIES={SITE_THEME_COOKIE: "wix"})
    assert site_theme(request) == {"site_theme": "wix"}


def test_invalid_cookie_falls_back_to_modern():
...
def test_set_site_theme_normalizes_unknown_theme(client):
    ...
    assert resp.cookies[SITE_THEME_COOKIE].value == "modern"
